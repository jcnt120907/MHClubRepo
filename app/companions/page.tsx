...
function MergeCompanion({source,items,onClose,onDone}:{source:Companion;items:Companion[];onClose:()=>void;onDone:(message:string)=>void}){const [query,setQuery]=useState(""),[target,setTarget]=useState<Companion|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState("");const candidates=items.filter(x=>x._id!==source._id&&x.name.toLocaleLowerCase().includes(query.toLocaleLowerCase()));const submit=async()=>{if(!target)return setError("请搜索并选择要保留的陪陪");setBusy(true);const r=await fetch("/api/companions/"+source._id+"/merge",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({targetId:target._id})});const d=await r.json();setBusy(false);if(!r.ok)return setError(d.error);onDone("已合并「"+source.name+"」到「"+d.target+"」")};return <Modal title="合并陪陪资料" onClose={()=>!busy&&onClose()}><div className="merge-editor"><div className="merge-intro"><strong>合并前请确认资料</strong><p>订单与存单将转移到保留的陪陪；来源资料会删除。</p></div><div className="merge-source"><span>来源陪陪</span><b>{source.name}</b><small>将被合并并删除</small></div><label className="merge-search">保留陪陪<input autoFocus value={query} onChange={e=>{setQuery(e.target.value);setTarget(null)}} placeholder="输入名字搜索"/></label>{query&&!target&&<div className="merge-candidates">{candidates.length?candidates.map(x=><button type="button" key={x._id} onClick={()=>{setTarget(x);setQuery(x.name)}}><b>{x.name}</b><span>{x.paymentMethod||"未填写付款方式"}</span></button>):<p>没有符合的陪陪</p>}</div>}{target&&<div className="merge-target"><span>保留陪陪</span><b>{target.name}</b><small>{target.paymentMethod||"未填写付款方式"} · 付款资料会保留</small></div>}{error&&<div className="form-error">{error}</div>}<div className="dialog-actions"><button className="secondary" disabled={busy} onClick={onClose}>取消</button><button className="danger" disabled={busy||!target} onClick={submit}>{busy?"合并中…":"确认合并"}</button></div></div></Modal>}

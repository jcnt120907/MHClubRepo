...
function TelegramImport({
  ...
  const [items, setItems] = useState<Array<{ requestedOrderNo?: string; storageMinutes?: number; ownerSource?: "IG" | "Telegram"; ownerId?: string; input?: Input; errors: string[]; warnings: string[] }>>([]);
  ...
  return <Modal title="Telegram 批量报单" onClose={() => !busy && onClose()}><div className="form-body"><p className="field-help">贴上多笔 Telegram 报单，系统会保留单号、自动计算时长与附加项目；新陪陪会自动加入名单。</p><textarea className="telegram-input" rows={9} placeholder="1. 单号：P0006 陪陪：小酒窝 ... 日期：17/9/2026" value={text} onChange={e => { setText(e.target.value); setItems([]); }}/>{items.length > 0 && <div className="import-preview">{items.map((item,index)=><div className={item.errors.length ? "import-row invalid" : "import-row"} key={index}><strong>{item.requestedOrderNo || "未识别单号"}</strong>{item.input && <span>{item.input.companion} · {item.input.customerService} · {item.input.service} · {item.input.quantity || "礼物"} · 基础 RM {item.input.unitPrice}{item.input.addons.length ? " · 附加 " + item.input.addons.map(k=>addons[k].label+" +RM"+addonPrice(k,item.input!.service)).join("、") : ""} · 服务小计 RM {calculate(item.input).total-item.input.gift} · 礼物 RM {item.input.gift}{item.storageMinutes ? " · 存单 " + item.storageMinutes + " 分钟" : ""}{item.ownerSource && item.ownerId ? " · 老板 " + item.ownerSource + " " + item.ownerId : ""}</span>}{item.errors.map(message=><small className="field-error" key={message}>{message}</small>)}{item.warnings.map(message=><small className="field-help" key={message}>{message}</small>)}</div>)}</div>}{error&&<div className="form-error">{error}</div>}</div><div className="dialog-actions"><button className="secondary" disabled={busy} onClick={onClose}>取消</button><button className="secondary" disabled={busy||!text.trim()} onClick={()=>preview(false)}>{busy?"处理中…":"解析预览"}</button><button className="primary" disabled={busy||!valid} onClick={()=>preview(true)}>{busy?"导入中…":"确认导入"}</button></div></Modal>;
}
function Editor({
  order,
  onClose,

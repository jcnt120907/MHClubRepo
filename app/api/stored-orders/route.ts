...
export async function POST(req:NextRequest){try{const v=await req.json();return NextResponse.json(await createStoredOrder(v.orderId,v.durationMinutes),{status:201})}catch(e){return NextResponse.json({error:e instanceof Error?e.message:"存单保存失败"},{status:400})}}
